...
int main() {
	...
	// конструкторы cо списком инициализации.
	//=====================================================
	begin_section("Оператор присваисвания");
	{
		counter("К матрице A[3x3] присваиваится матрица B[2x2]");
		...
		A = B;
		std::cout << "(A := B) = " << A;
		counter("К матрице A[3x3] присваивается вектор-строка {-1,-1,2}");
		RowVector<int> RV = { -1, -1, 2 };
		A = RV;
		std::cout << A;
		counter("К матрице A[3x3] присваиваетсся вектор-столбец {0, 9 ,1}");
		ColVector<int> CV = { 0, 9, 1 };
		A = CV;
		std::cout << A;
	}
	end_section();
	...
	begin_section("Дополнительный тест.(типы данных и транспонирование)");
	{
		Matrix<double> m =
		{ {1., 0., 0.},
		  {4., 5., 6.},
